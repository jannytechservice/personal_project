...
export const getScopes = (scopeSet: string): string => {
  if (scopeSet === 'PROFILE') {
    return 'openid profile email offline_access';
  } else if (scopeSet === 'ACCOUNTING') {
    return 'openid profile email offline_access accounting.contacts accounting.transactions accounting.settings accounting.attachments';
  } else {
    return '';
  }
};
...
export const getRedirectUri = (xeroEnv: string): string => {
  if (xeroEnv === 'LOCAL') {
    return 'https://localhost:4200/xero-redirect';
  } else {
    return '';
  }
};

export const initializeXeroClientWithTokenSet = async (
  tokenSet: TokenSet
): Promise<XeroClient> => {
  let xero: XeroClient;
  try {
  ...
  try {
    ...
    console.log('Success setTokenSet: ', xero);
  } catch (err: any) {
    console.error('ERROR setTokenSet: ', err);
    throw new Error(err.message);
  }

  if (isTokenExpired(Number(tokenSet.expires_at))) {
    try {
      await xero.refreshToken();
      console.log('Success refreshToken: ', xero);
    } catch (err: any) {
  ...
  try {
    await xero.updateTenants(false);
    console.log('Success update xero tenants: ', xero.tenants);
  } catch (err: any) {
