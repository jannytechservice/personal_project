...
export function MfaForm() {
  ...
  useEffect(() => {
    const checkMFA = async () => {
      ...
      if (output) {
        setMfaEnabled(output.preferred === 'TOTP');
      }
    };
  ...
  const setUp2Fa = async () => {
    ...
    try {
      ...
      setCode(sharedSecret);
      setTwoFACode(
        `otpauth://totp/AWSCognito:${sub}?secret=${sharedSecret}&issuer=${MFA_ISSUER} (${email})`
      );
    } catch (err) {
