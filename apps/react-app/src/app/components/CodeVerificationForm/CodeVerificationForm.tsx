  ...
  const confirmEmail = async () => {
    ...
    try {
      await confirmUserAttribute({
        userAttributeKey: codeType,
        confirmationCode: watch('code'),
      });
      await updateUser({
        variables: {
          input: {
      ...
      await updateUserAttributes({
        userAttributes: { [codeType]: watch(updateName) },
      });
      ...
      >
        ...
        <DialogContent>
          ...
          {authError?.name && (
            <WBAlert
              title={
                i18n.exists(authError.name)
                  ? t(authError.name, { ns: 'authentication' })
                  : authError?.message
              }
              severity="error"
            />
